"""
elasticnet_feature_selection_with_plots.py
------------------------------------------
Eksik verili ve kategorik özellikli bir veri setinde, hedef 'qe(mg/g)' için
Elastic Net (ElasticNetCV) ile özellik seçimi yapar. Sıfır olmayan katsayıları
çıkarır ve çeşitli görselleştirmeler üretir.
...
print(f"Selected alpha: {model.named_steps['clf'].alpha_:.6f}")
print(f"Selected l1_ratio: {model.named_steps['clf'].l1_ratio_:.3f}")
...
print("\n=== Non-zero Elastic Net coefficients (sorted by |coef|) ===")
if selected.empty:
    print("(No non-zero coefficients selected — try a wider l1_ratio grid or fewer alphas.)")
else:
    print(selected.head(40).to_string(index=False))

# === EK 1: coef=0 olanları DA yazdır ===
zero_df = (coef_df.loc[coef_df["Coef"] == 0]
                    .sort_values(["Feature"]))
print("\n=== Zero Elastic Net coefficients (|coef|==0) ===")
if zero_df.empty:
...
# with pd.ExcelWriter(OUT_DIR / "elasticnet_selected_features.xlsx", engine="xlsxwriter") as wr:
...
colors = sel_top["Coef"].apply(lambda v: "#1f77b4" if v >= 0 else "#d62728")  # mavi/ kırmızı
plt.barh(sel_top["FeatShort"], sel_top["Coef"], color=colors, edgecolor="black")
plt.axvline(0, color="black", linewidth=0.8)
plt.title(f"Elastic Net Coefficients (Top {len(sel_top)})")
plt.xlabel("Coefficient")
plt.tight_layout()
plt.savefig(OUT_DIR / "elastic_coef_signed_barh.png", dpi=300, bbox_inches="tight")
plt.close()
...
plt.title(f"Lollipop — Elastic Net Coefficients (Top {len(sel_top)})")
plt.xlabel("Coefficient")
plt.tight_layout()
plt.savefig(OUT_DIR / "elastic_coef_lollipop.png", dpi=300, bbox_inches="tight")
plt.close()
...
plt.xlabel("|Coefficient|")
plt.tight_layout()
plt.savefig(OUT_DIR / "elastic_coef_abs_barh.png", dpi=300, bbox_inches="tight")
plt.close()
...
plt.tight_layout()
plt.savefig(OUT_DIR / "elastic_actual_vs_pred_train.png", dpi=300, bbox_inches="tight")
plt.close()
...
plt.tight_layout()
plt.savefig(OUT_DIR / "elastic_residuals_hist.png", dpi=300, bbox_inches="tight")
plt.close()
...
# === EK 2: OHE SONRASI (kategorikler dahil) BET korelasyonları ===
try:
