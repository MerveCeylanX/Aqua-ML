"""
imputed_first_six_algorithms.py
--------------------------------------------------------
ML: 6 model, 5-fold CV, tek figürde 6 subplot (RMSE fix)
--------------------------------------------------------
...
5) 6 farklı regresyon modeli kurar:
   - Ridge, Lasso, ElasticNet, RandomForest, GradientBoosting, SVR
6) Her model için:
...
10) Çıktılar:
    - Grafik → ./figures/imputed_first_six_algorithms.png
...
pre = ColumnTransformer(transformers=[("num", num_transformer, num_feats),("cat", cat_transformer, cat_feats)], remainder="drop")

# --- Modeller ---
models = {
    "Ridge": Ridge(alpha=1.0, random_state=RANDOM_STATE),
    "Lasso": Lasso(alpha=1e-3, max_iter=10000, random_state=RANDOM_STATE),
    "ElasticNet": ElasticNet(alpha=1e-3, l1_ratio=0.5, max_iter=10000, random_state=RANDOM_STATE),
...
for ax, (name, reg) in zip(axes, models.items()):
    ...
    ax.scatter(y_train, yhat_tr, s=18, alpha=0.65, color="#03C2FB",label="Train" if name=="Ridge" else None, edgecolors="none")
    ax.scatter(y_test,  yhat_te, s=30, alpha=0.85, color="#001583", marker="^",label="Test"  if name=="Ridge" else None, edgecolors="none")
...
out_fig = FIG_DIR / "imputed_first_six_algorithms.png"
...
print("\n=== Özet (CV ve Test) ===")
print(res_df.sort_values("test_r2", ascending=False).to_string(index=False, float_format=lambda x: f"{x:.3f}"))
