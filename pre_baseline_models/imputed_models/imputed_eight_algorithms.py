"""
imputed_eight_algorithms.py
--------------------------------------------------------
ML: 8 model, 5-fold CV, tek figürde 8 subplot (RMSE fix)
--------------------------------------------------------
...
5) 8 farklı regresyon modeli kurar:
   - ExtraTrees, HistGradientBoosting, KNN, AdaBoost, MLP, KernelRidge, Linear, Huber
6) Her model için:
   - 5-fold CV (R², RMSE, MAE) skorlarını hesaplar.
   - Train ve Test metriklerini yazdırır.
   - Tahmin vs Gerçek scatter grafiklerini tek figürde (3x3 subplot) çizer.
...
10) Çıktılar:
    - Grafik → ./figures/imputed_eight_algorithms.png
...
models = {
    "ExtraTrees": ExtraTreesRegressor(
        ...
        random_state=RANDOM_STATE, n_jobs=1  # iç paralellik kapalı (loky çakışmasın)
    ),
    ...
    "Huber": HuberRegressor(alpha=1e-4, max_iter=2000),
}
...
fig, axes = plt.subplots(3, 3, figsize=(18, 12))  # 8 model için 3x3 grid
axes = axes.ravel()
...
for ax, (name, reg) in zip(axes, models.items()):
    ...
    ax.scatter(y_train, yhat_tr, s=18, alpha=0.65, color="#FECB04",
               label="Train" if name=="ExtraTrees" else None, edgecolors="none")
    ax.scatter(y_test,  yhat_te, s=30, alpha=0.85, color="#1E5AF9",
...
out_fig = FIG_DIR / "imputed_eight_algorithms.png"
