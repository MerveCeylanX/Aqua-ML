"""
not_imputed_xgboost_hyper_SHAP.py
----------------------------------------------------
ML Workflow: XGBoost + Hyperparameter Optimisation + SHAP  (One-Hot for Activation_Atmosphere)
----------------------------------------------------
- Farmasötik özellikler (E,S,A,B,V) ve elemental molar oranlar (C_molar, H/C, O/C, N/C, S/C) eklenir.
- 'Activation_Atmosphere' tek kategorik kolonu güvenli şekilde one-hot yapılır
  (örn. activation_atmosphere_air, activation_atmosphere_n2, ...), dummy kolonlar X'e eklenir.
- NaN'lar için imputation yapılmaz; XGBoost sayısal kolonlardaki NaN'ları kendi yönetir.
- XGBoost hiperparametre optimizasyonu RandomizedSearchCV ile yapılır (5-fold CV, R2 skoru).
- En iyi model için Train/Test ve CV metrikleri, OOF sonuçları hesaplanır.
- En iyi model için SHAP summary ve importance (bar) grafikleri üretilir; one-hot dummy'ler ayrı özellik olarak raporlanır.
- Çıktılar:
    figures/ml_results_xgboost_best.png
    figures/shap_summary_xgboost_best.png
    figures/shap_importance_xgboost_best.png
    results/ml_results.xlsx  (ML_Summary, Best_Params_XGB, XGB_CV_Results)
"""
...
# -------------------- KULLANICI AYARLARI --------------------
IN_PATH      = r"D:\Aqua_ML\data\Raw_data.xlsx"   # Ham veri girişi (istersen ./data/ altına da koyabilirsin)
RANDOM_STATE = 42
TEST_SIZE    = 0.2
...
# Veriler UFZ LSER-Database'den derlenmiştir.
...
# Sayısal adaylar (varsa X'e alınacak)
num_feats = [
    "Agent/Sample(g/g)",
    "Soaking_Time(min)",
    "Soaking_Temp(K)",
...
# Sadece mevcut sütunları al (verinizde olmayan isimler filtrelenir)
...
# -------------------- 5) XGBoost — HPO + En İyi Model --------------------
from xgboost import XGBRegressor

xgb_base = XGBRegressor(
    booster="gbtree",
    tree_method="hist",
    enable_categorical=False,   # one-hot sonrası: tüm girdiler numerik
...
print(f"[HPO] En iyi R2 (CV): {best_cv_r2:.3f}")
print("[HPO] En iyi parametreler:", best_params)

# CV'yi çoklu metrikle tekrar raporla (R2, RMSE, MAE)
scoring = {"r2":"r2", "neg_rmse":"neg_root_mean_squared_error", "neg_mae":"neg_mean_absolute_error"}
with parallel_backend("threading", n_jobs=N_JOBS):
    cvres_best = cross_validate(best_model, X_train, y_train, cv=cv, scoring=scoring,
                                n_jobs=N_JOBS, return_train_score=False)

fold_r2   = cvres_best["test_r2"]
fold_rmse = -cvres_best["test_neg_rmse"]
fold_mae  = -cvres_best["test_neg_mae"]

cv_r2, cv_rmse, cv_mae = fold_r2.mean(), fold_rmse.mean(), fold_mae.mean()
print(f"[CV] XGBoost (best) | R2={cv_r2:.3f} | RMSE={cv_rmse:.3f} | MAE={cv_mae:.3f}")

# Fit & Test (en iyi model)
...
# --- SCATTER GRAFİĞİ (best model) -> ./figures/ ---
...
plt.scatter(y_train, yhat_tr, s=18, alpha=0.65, color="#0014E7", label="Train", edgecolors="none")
plt.scatter(y_test,  yhat_te, s=30, alpha=0.85, color= "#03C2FB", marker="^", label="Test", edgecolors="none")
plt.plot([lo,hi],[lo,hi],"--",lw=1.0)
plt.xlim(lo,hi); plt.ylim(lo,hi)
plt.title("XGBoost (Best from HPO)")
...
out_fig = FIG_DIR / "ml_results_xgboost_best.png"
plt.tight_layout(); plt.savefig(out_fig, dpi=200, bbox_inches="tight"); plt.close()
print(f"[OK] Grafik kaydedildi: {out_fig}")

# --- OOF (best model) ---
with parallel_backend("threading", n_jobs=N_JOBS):
    oof_pred = cross_val_predict(best_model, X_train, y_train, cv=cv, n_jobs=N_JOBS)
...
print(f"OOF (best): R2={oof_r2:.3f} | RMSE={oof_rmse:.3f} | MAE={oof_mae:.3f}")

# --- METRİKLER + EN İYİ PARAMETRELER -> ./results/ml_results.xlsx ---
try:
    res_row = pd.DataFrame([{
        "model": "XGBoost (best, one-hot Activation_Atmosphere)",
    ...
    with pd.ExcelWriter(RESULTS_XLSX, mode="w", engine="openpyxl") as xw:
        ...
        pd.DataFrame([best_params]).to_excel(xw, sheet_name="Best_Params_XGB", index=False)
        pd.DataFrame(search.cv_results_).to_excel(xw, sheet_name="XGB_CV_Results", index=False)
    print(f"[OK] Metrikler ve en iyi parametreler yazıldı: {RESULTS_XLSX}")
except Exception as e:
    print(f"[Uyarı] Sonuç Excel yazımında sorun: {e}")

# -------------------- 6) SHAP (interventional, NaN OK, one-hot ile uyumlu) --------------------
try:
    import shap
    ...
    def to_numeric_array(df_like):
        if isinstance(df_like, pd.DataFrame):
            arr = df_like.to_numpy(dtype=np.float64, copy=False)
        else:
    ...
    # -------- XGBoost için SHAP (best_model ile) --------
    ...
    try:
        explainer = shap.TreeExplainer(best_model, X_bkg, feature_perturbation="interventional")
        shap_values = explainer(X_te_full)
    except Exception:
        explainer = shap.TreeExplainer(best_model, feature_perturbation="interventional")
        shap_values = explainer(X_te_full)

    out_shap = FIG_DIR / "shap_summary_xgboost_best.png"
    shap.summary_plot(
    ...
    plt.xlabel("Ortalama |SHAP| (XGBoost Best, TreeExplainer)")
    plt.title("Özellik Önemi (SHAP, kategorik seviyeler ayrı)")
    plt.tight_layout()
    
    out_shap_bar = FIG_DIR / "shap_importance_xgboost_best.png"
